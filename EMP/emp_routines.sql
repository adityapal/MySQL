...
DELIMITER ;;
CREATE DEFINER=`user`@`localhost` FUNCTION `EMP_SALARY`(EMPID INT) RETURNS int(11)
BEGIN
    -- SELECT EMP.EMPSALARY(7782)
    RETURN (SELECT DISTINCT SAL FROM EMP.EMP WHERE EMP.EMPNO = EMPID);
  END ;;
DELIMITER ;
